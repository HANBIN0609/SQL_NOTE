...
-- 사번, 이름, 입사일, 부서코드 조회
-- 별칭 사용
-- 날짜 데이터는 기록된 포맷과 일치되게 작성하면 됨
-- 작은 따옴표로 묶어서 표현함 : '1995/01/01' 또는 '95/01/01'
...
-- SELECT 절에서 조회한 컬럼값들의 연결 처리를 해서 하나의 ㅇ문장을 만들거나, 
-- 컬럼값 뒤에 단위 등을 표시할 때 이용할 수 있음
SELECT EMP_NAME  || ' 직원의 급여는 ' || SALARY || '원 입니다.' 급여정보
FROM EMPLOYEE
WHERE DEPT_ID = '90';
...
-- 컬럼의 값이 작은값 이상이면서 큰값 이하인 행들을 골라라. 라는 의미임
-- WHERE 컬럼명 >= 작은값 AND 컬럼명 <= 큰값  과 같은의미
...
-- LIKE 연산자
-- 문자열 값에 대한 패턴을 제시해서 패턴과 일치하는 문자열을 골라낼 때 사용하는 연산자
-- WHERE 컬럼명 LIKE '문자패턴'
-- 문자패턴에 와일드카드 문자  사용함 : % (0개 이상의 글자), _ (글자 한자리)
...
-- 기록값인 '_' 와 와일드카드 '_' 가 구분이 안됨 >>> 전부 다 와일드카드 '_' 로 해석됨
-- 기록값을 구분하기 위해서 ESCAPE OPTION 을 사용하면 됨
-- 기록문자 앞에 기호를 하나 표시함, 무슨 기호든 상관 없음
...
-- WHERE DEPT_ID = NULL; -- 결과가 안나오는 에러임
...
-- 부서는 배정받지 않았는데, 관리자는 있는 직원
-- 사번, 이름, 관리자사번, 부서코드
SELECT EMP_ID, EMP_NAME, MGR_ID, DEPT_ID
FROM EMPLOYEE
WHERE DEPT_ID IS NULL 
AND     MGR_ID IS NOT NULL; -- 0개
...
SELECT EMP_ID 사번, EMP_NAME 사원명, BONUS_PCT 보너스포인트, PHONE 번화번호
FROM EMPLOYEE
...
--*****************************************************************************************************************
-- 함수 (FUNCTION)
-- 컬럼에 기록된 값을 읽어서 처리한 결과를 반환하는 형태
-- 함수명(컬럼명) 으로 사용함
-- 단일행 함수와 그룹 함수로 구분됨
-- 단일행 함수 : 읽은 값이 N개이면 반환값도 N개
-- 그룹 함수 : 읽은 값이 N개여도 반환값은 1개 
...
-- 해결 1 : 급여 평균을 따로 구한 다음, 그 값을 WHERE 절에서 사용
SELECT AVG(SALARY)
FROM EMPLOYEE;  -- 2961818.1818......

SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY > 2961818.1818; 

-- 해결 2 : 서브쿼리를 사용(SELECT 문 안에 SELECT 문 사용)
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE); 

-- 단일행 함수와 그룹함수는 SELECT 절에 함께 사용 못함
-- ORDB (Object Relational DataBase) : 객체 관계형 데이터베이스
-- 2차원 테이블 구조로 저장 데이터를 표현함. (SELECT 한 RESULT SET은 사각형이어야 함)
...
FROM DUAL;
-- 테이블 참조없이 단순한 계산의 결과만 SELECT 하는 경우, 더미(DUMMY) 테이블을 FROM 절에 사용할 수 있음
-- 오라클에서 제공함
...
-- INSTR('문자열리터럴' | 문자가 기록된 컬럼명, 찾을 문자, 찾을 시작위치, 몇 번째 문자) 함수
...
-- LPAD('문자열리터럴' | 문자가 기록된 컬럼명, 출력할 너비 글자 수, 남는 영역 채울 문자) >>> 오른쪽 정렬
-- RPAD('문자열리터럴' | 문자가 기록된 컬럼명, 출력할 너비 글자 수, 남는 영역 채울 문자) >>> 왼쪽 정렬
-- 채울 문자가 생략되면 기본값은 ' ' (공백문자)임
