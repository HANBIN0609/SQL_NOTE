...
-- 데이터베이스 객체를 생성/변경/제거에 사용되는 SQL 명려어 제공함
...
CREATE TABLE 테이블명(
        ...
        컬럼명 자료형(크기) [제약조건],
        컬럼명 자료형(크기) [CONSTRAINT 이름 제약조건종류],         -- 컬럼 레벨 : 컬럼옆에 지정.
        컬럼명 자료형(크기) [DEFAULT 기본값 CONSTRAINT 저장이름 제약조건종류] -> 여러개 올 떄 순서대로 넣어줘야함
        컬럼명 자료형(크기) [CONSTRAINT 저장이름] 제약조건1 [CONSTRAINT 저장이름] 제약조건2...
            -- 테이블 레벨 : 컬럼 구성한 다음에 제약조건만 따로 지정할 수도 있음.
        제약조건종류 (적용할 컬럼명),
        CONTRAINT 저장할이름 제약조건종류 (적용할 컬럼명)
);
*/
...
CREATE TABLE TEST (
        ...
        AGE NUMBER, -- 숫자는 크기지정 생략 가능, 기본 7자리로 설정됨
        ENROLL_DATE DATE -- 날짜는 크기지정 없음
);
...
VALUES (기록할 값, 기록할 값, ....);  
주의사항 : 컬럼 나열 순서에 맞춰서 값을 기술해야함. 나열된 컬럼과 기술할 값의 자료형과 갯수가 일치해야함 
...
-- 컬럼에 NULL(빈 칸) 사용 못한다는 의미임
-- 컬럼레벨에서만 설정할 수 있음 (테이블레벨에서 설정 못함)
...
INSERT INTO TESTNN
VALUES (NULL, '자바'); -- ERROR : NOT NULL 제약조건 위배됨

INSERT INTO TESTNN (NN_NAME) -- 제외된 컬럼은 NULL 이 됨
VALUES ('ORACLE'); -- ERROR : NOT NULL 제약조건 위배됨

INSERT INTO TESTNN
VALUES (2, NULL); -- 행 추가

INSERT INTO TESTNN (NNID) -- 명시되지 않은 컬럼은 NULL 들어감
VALUES (3); -- 행 추가
...
CREATE TABLE TESTNN2 (
        ...
        NN_NAME VARCHAR2(20),
        -- 테이블레벨
        CONSTRAINT NN_TN2_ID NOT NULL (NN_ID) -- ERROR : NOT NULL 은 테이블레벨에서 설정 불가
);
...
INSERT INTO TESTUN VALUES ('AAA', '자바');  -- ERROR : 기록되어 있는 값 중복 기록 
...
-- 중복 판단 : '100 오라클' != '200 오라클' -> INSERT 됨
INSERT INTO TESTUN3 VALUES ('200', '오라클', '02'); 
-- ERROR : '200 오라클' = '200 오라클', 중복

-- UNIQUE 복합키에 대한 NULL 적용
INSERT INTO TESTUN3 VALUES (NULL, '오라클', '03');
INSERT INTO TESTUN3 VALUES ('200', NULL, '03'); -- NOT NULL 제약조건에 위배됨
...
INSERT INTO TESTPK VALUES (1, '홍길동', '15/0312');
INSERT INTO TESTPK VALUES (2, '김유신', TO_DATE('17/05/10'));
INSERT INTO TESTPK VALUES (NULL, '이순신', SYSDATE); -- ERROR : NULL 사용 불가
...
INSERT INTO TESTPK4 VALUES (100, '오라클', SYSDATE); -- ERROR : '100 오라클' = '100 오라클', 중복
-- NULL 사용 못함
...
-- 제한값은 반드시 리터럴(값 자체)만 사용할 수 있음. *함수 사용 불가 -> 사용될 때마다 바뀌는 값은 사용 불가*

CREATE TABLE TESTCHK (
        C_NAME VARCHAR2(15) CONSTRAINT NM_TCK_NAME NOT NULL,
...
INSERT INTO TESTCHK VALUES ('LG G7', 1250000, 'B'); -- ERROR : C_PRICE 에 제한된 값의 범위를 초과함
INSERT INTO TESTCHK VALUES ('LG G7', 0, 'B'); -- ERROR : C_PRICE 에 제한된 값의 범위를 초과함
INSERT INTO TESTCHK VALUES ('LG G7', 7500, 'D'); -- ERROR : C_LEVEL 에 허용되지 않은 값 사용 
INSERT INTO TESTCHK VALUES ('LG G7', 7500, 'a'); -- ERROR : C_LEVEL 에 허용되지 않은 값 사용, 값은 대소문자 구분함
...
CREATE TABLE TESTCHK2 (
        ...
        -- C_DATE DATE CHECK (C_DATE < SYSDATE) -- ERROR : 제한값이 바뀌는 함수는 사용 못함
...
-- REFERENCES 할 컬럼(값제공컬럼)은 반드시 PRIMARY KEY | UNIQUE 제약조건이 설정되어야함
...
INSERT INTO TESTFK VALUES ('333', '90', 'J4'); -- ERROR : EMP_ID 에서 없는 값을 참조했기 때문, 부모키가 없음
INSERT INTO TESTFK VALUES ('210', '70', 'J4'); -- ERROR : DEPT_ID 에서 없는 값을 참조했기 때문, 부모키가 없음
INSERT INTO TESTFK VALUES ('210', '80', 'j4'); -- ERROR : JOB_ID 에서 없는 값을 참조했기 때문, 부모키가 없음 
INSERT INTO TESTFK VALUES (NULL, '80', 'J4'); -- NULL 사용가능
INSERT INTO TESTFK VALUES ('124', NULL, 'J7'); -- NULL 사용가능
INSERT INTO TESTFK VALUES ('124', '30', NULL); -- NULL 사용가능
INSERT INTO TESTFK VALUES (NULL, NULL, NULL); -- 셋 다 NULL 사용가능
...
CREATE TABLE TESTFK2 (
        FID CHAR(3) REFERENCES TEST_NOPK, -- 참조컬럼 생략되면 자동으로 PK 컬럼이 참조됨
        FNAME VARCHAR2(15)
); -- ERROR : 참조테이블에 기본 키가 없음
...
CREATE TABLE TESTKF3 (
        FID CHAR(3) REFERENCES TEST_NOPK3,
...
-- 기본적으로 FOREIGN KEY 제약조건이 설정되면, 값을 제공하는 참조테이블의 컬럼값은 함부로 삭제 못함
-- 참조컬럼의 값이 자식레코드 쪽에서 사용되고 있으면 삭제 못함
-- 즉, EMPLOYEE.DEPT_ID 에 기록된 '90' 부서코드가 있으면, DEPARTMENT.DEPT_ID 의 '90' 부서는 삭제 못한다는 의미

-- DML : DELETE 문
-- 행 삭제 구문임 (행 갯수가 줄어듬)
-- 사용형식 : DELETE FROM 테이블명 WHERE 컬럼명 비교연산자 삭제할조건값;
-- 조건(WHERE)에 해당되는 값이 있는 행을 삭제하라는 의미임
...
-- 부모키를 삭제할 수 있게 됨
-- ON DELETE SET NULL : 부모키가 삭제되면, 자식레코드(값사용컬럼)의 값을 NULL로 바꿈
-- ON DELETE CASCADE : 부모키가 삭제되면, 자식레코드가 있는 행을 같이 삭제함
-- ON DELETE RESTRICTED : 기본값, 삭제 불가능을 의미함
...
-- ON DELETE SET NULL : 부모키가 삭제되면 자식레코드 값을 NULL로 바꿈
...
-- 삭제 취소 : ROLLBACK (TCL : Transaction Control Language)
ROLLBACK;
...
SELECT * FROM PRODUCT_STATE; -- 부모키
SELECT * FROM PRODUCT2; -- 자식레코드

-- 부모키(값제공컬럼)가 복합키(여러 개의 컬럼을 하나로 묶은 것)이면
-- FOREIGN KEY 제약조건 설정하는 컬럼(자식레코드)도 같은 복합키여야함
-- 복합키를 단일키로 사용 못함, 단일키를 FOREIGN KEY 제약조건에서 복합키로 만들 수도 없음
...
CREATE TABLE TESTFK4 (
    FID NUMBER,
    FNAME VARCHAR2(10),
    -- FOREIGN KEY (FID) REFERENCES TEST_COMP (ID) -- ERROR : 복합키를 따로 단일키로 사용 못함
    FOREIGN KEY (FID, FNAME) REFERENCES TEST_COMP -- 복합키는 복합키로 설정해야함    
);
