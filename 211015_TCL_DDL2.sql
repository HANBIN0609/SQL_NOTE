...
SELECT * FROM DEPARTMENT;       -- INSERT 취소 : '40' 부서 없어짐
...
COMMIT; -- 변경사항 최종 저장, 동시 접속 시 다른 사용자는 변경사항을 볼 수 없음
...
START WITH 300 -- 시작값 : 300 부터  시작
INCREMENT BY 5 -- 증가값 : 5 씩 증가
MAXVALUE 310 -- 310 까지 생성
NOCYCLE -- 310 생성 후 더 이상 생성 안함
...
-- 시퀀스 사용 : 시퀀스이름.NEXTVAL 을 사용해야 숫자가 발생
-- INSERT 문에서 값 기록시에 주로 사용함
...
INCREMENT BY 5
MAXVALUE 15
CYCLE -- 시작값으로 시작해서 MAXVALUE에 도달하면 이후 값 순환시에는 무조건 1부터 시작됨
NOCACHE;
...
-- 현재 시퀀스 숫자 확인 : 시퀀스이름.CURRVAL (반드시 NEXTVAL 한번 실행한 다음에 사용해야됨)
SELECT SEQ_EMPID2.CURRVAL FROM DUAL;

CREATE SEQUENCE SEQ_EMP3
START WITH 300
INCREMENT BY 5
MAXVALUE 310
NOCYCLE
...
SELECT SEQ_EMP3.CURRVAL FROM DUAL; -- ERROR 
-- NEXTVAL 을 한번도 사용하지 않은 상태에서는 CURRVAL 사용할 수 없음

SELECT SEQ_EMP3.NEXTVAL FROM DUAL;
SELECT SEQ_EMP3.CURRVAL FROM DUAL;
...
-- 시퀀스 수정
/*
- START WITH 는 수정 못함, 나머지는 모두 수정할 수 있음.
- START WITH 를 바꾸려면, 시퀀스 객체 삭제하고 다시 만들기해야함
- 변경 내용은 이후 사용시에 적용됨
...
AS 서브쿼리;        -- 해당 서브쿼리가 뷰에 저장할 SELECT 문
*/
...
-- 처음 실행시 "insufficient privilleges" 에러 발생하면
...
-- PRIMARY KEY 와 UNIQUE 의 제약조건에 대한 INDEX 는 자동적으로 생성됨
/*
