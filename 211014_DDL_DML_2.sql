...
-- TABLE_SUBQUERY1 테이블에 저장 처리하는 구문을 작성 실행하시오
CREATE TABLE TABLE_SUBQUERY1
AS
SELECT EMP_ID 사번, EMP_NAME 이름, SALARY 급여, DEPT_NAME 부서명, JOB_TITLE 직급명
FROM EMPLOYEE
...
SELECT * FROM EMP_MAN;
DESC EMP_MAN;
-- 여자 직원들의 정보만 골라내서, EMP_FEMALE 테이블에 저장함
...
-- DEPT_NAME, JOB_TITLE, EMP_NAME, EMP_ID 순으로 구성함
...
-- 주의사항 : 서브쿼리 SELECT 절의 컬럼 갯수와 바꿀 컬럼명의 갯수가 일치해야함
/*
...
-- 데이터 딕셔너리 (데이터 사전)
-- 사용자가 생성한 모든 데이터베이스 객체 정보가 테이블 형태로 자동 저장관리되는 영역
-- 조회만 할 수 있고, 수정은 못함
-- DBMS 시스템에 의해 자동 관리되고 있음
-- 예를 들면, 사용자가 설정한 제약조건도 자동 저장 관리되고 있음 : USER _CONSTRAINTS
...
-- 주의사항 : 서브쿼리의 각 컬럼별 결과값과 맞추어서 제약조건 추가해야함
...
-- 성해교 직원의 같은 직급, 같은 급여로 수정 처리 하시오
...
컬럼명 나열이 생략되면 테이블이 가진 모든 컬럼에 값을 기록한다는 의미
주의사항 : 나열된 컬럼과 기록할 값의 갯수, 순서, 자료형 등이 일치해야함
                논리적 오류도 주의해야함 (EMP_NO <= '홍길동' 기록되면 안됨. 에러(물리적 오류)는 나지않지만 논리적 오류 발생)
*/
-- 논리적 오류, 주민번호 값에 이름, 이름에 주민번호가 들어감
INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, HIRE_DATE, JOB_ID, SALARY,
                                    BONUS_PCT, MARRIAGE, MGR_ID, DEPT_ID)
VALUES ('900', '811225-2345678', '오윤하', 'oyua@kkk.com', '0101234568','06/01/01', 'J7', 2500000, 0, 'N', '176', '90');
...
-- CREATE, ALTER, DROP
-- 데이터베이스 객체를 만들고(CREATE), 수정하고(ALTER), 삭제(DROP)하는 구문임
...
SELECT * FROM EMPLOYEE;
-- 서브쿼리를 이용한 테이블 생성시에는
-- 컬럼명, 자료형(크기), NOT NULL 제약조건, DATA 만 복사됨
-- 나머지 제약조건들과 DEFAULT 는 복사가 안됨
...
-- 컬럼 자료형 변경
-- 값이 비어있는 컬럼은 아무 자료형으로나 변경 가능함
-- 값이 기록되어 있는 경우에는 문자형만 문자형끼리 변환 가능함. CHAR <=> VARCHAR2
-- 크기는 같거나 크게 변경할 수 있음
ALTER TABLE EMP2
MODIFY (EMP_ID VARCHAR2(3), EMP_NAME CHAR(20));
...
ALTER TABLE DEPT_COPY
DROP (LOC_ID, LNAME); -- 컬럼 여러개 제거

DESC DEPT_COPY;

-- 데이터베이스 테이블은 최소 1개의 컬럼을 가지고 있어야함
-- 컬럼이 없는  테이블은 존재할 수 없음
-- 모든 컬럼을 제거할 수는 없음
ALTER TABLE DEPT_COPY
DROP (DEPT_ID, DEPT_NAME);  -- ERROR 

CREATE TABLE TB1 (); -- ERROR : 컬럼이 없는 테이블은 만들어질 수 없음
...
CREATE TABLE TB1 (
        ...
        TFK NUMBER REFERENCES TB1, -- 참조하는 컬럼 생략 = 해당 테이블의 PRIMARY KEY를 참조
        COL1 NUMBER,
...
ALTER TABLE TB_EXAM
RENAME CONSTRAINT SYS_C007431 TO PK_TBE_EID;

ALTER TABLE TB_EXAM
RENAME CONSTRAINT SYS_C007432 TO FK_TBE_EID;
...
-- 참조되는 테이블(FOREIGN KEY 설정에 대한 부모키가 있는 테이블)은 제거 못함
CREATE TABLE DEPT (
...
DROP TABLE DEPT; -- EMP5 에서 참조하고 있기 때문에 제거(DROP) 못함
-- DEPT 에 대한 REFERENCES 제약조건을 함께 제거하면됨
